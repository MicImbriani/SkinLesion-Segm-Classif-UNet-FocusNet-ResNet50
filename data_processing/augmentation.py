...
def augment_operations(image_id, image_folder_path, mask_folder_path, train_val, x):
    ...
    transform = A.Compose(
        [
            A.ElasticTransform(
                alpha=30, sigma=120 * 0.05, alpha_affine=120 * 0.03, p=0.5
            ),
            A.CLAHE(p=1),
            A.GaussNoise(p=1),
            A.Resize(256, 256),
...
def augment_img(
    image_id, images_folder_path, masks_folder_path, csv_file_path, train_val
):
    ...
    if train_val == "Validation":
        ...
        img_1.save(
            images_folder_path + "/" + image_id + "x1" + ".png", "PNG", quality=100
        )
        img_1_mask.save(
        ...
        if melanoma == 1:
            ...
            img_2.save(
                images_folder_path + "/" + image_id + "x2" + ".png", "PNG", quality=100
            )
            img_3.save(
                images_folder_path + "/" + image_id + "x3" + ".png", "PNG", quality=100
            )
            img_4.save(
                images_folder_path + "/" + image_id + "x4" + ".png", "PNG", quality=100
            )
...
def augment_dataset(
    images_folder_path, masks_folder_path, csv_file_path, jobs, train_val
):
